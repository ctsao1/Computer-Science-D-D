...
public class CookieMonster {
	...
	public int queueCookies() {
		...
		while (!orphans.isEmpty()) {
			...
			if (validPoint(row, col + 1)) {
				orphans.offer(new OrphanScout(row, col + 1,
						orphans.peek().getCookiesDiscovered() + cookieGrid[row][col + 1]));
			}
			if (validPoint(row + 1, col)) {
				orphans.offer(new OrphanScout(row + 1, col,
						orphans.peek().getCookiesDiscovered() + cookieGrid[row + 1][col]));
			}
	...
	public int stackCookies() {
		...
		while (!orphans.empty()) {
			...
			if (validPoint(row, col + 1)) {
				orphans.push(new OrphanScout(row, col + 1,
						cookiesDiscovered + cookieGrid[row][col + 1]));
			}
			if (validPoint(row + 1, col)) {
				orphans.push(new OrphanScout(row + 1, col,
						cookiesDiscovered + cookieGrid[row + 1][col]));
			}
			if (!validPoint(row + 1, col + 1)) {
